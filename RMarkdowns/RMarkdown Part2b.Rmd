...
### determine specifications for simulation

# first, what are the data elements we want
dataNodes <- c('Istar', 'Rstar')
# get data nodes at each time point
(dataNodes <- sirModel$expandNodeNames(dataNodes, returnScalarComponents = TRUE))
# get all nodes that generate the data nodes
(parentNodes <- sirModel$getParents(dataNodes, stochOnly = TRUE))
# due to the temporal structure of the model, data from previous time points 
#   generates data at subsequent time points, so remove the data nodes from the 
#   list of parent nodes
(parentNodes <- parentNodes[-which(parentNodes %in% dataNodes)])
# not necessary here, but good practice to expand the parent node names for 
#   generality. Would be important if, e.g., you had a vector of betas beta[1:p]
(parentNodes <- sirModel$expandNodeNames(parentNodes, returnScalarComponents = TRUE))
# finally, get all dependencies of the parent nodes to determine everything
#   that needs to be updated during the simulation process
